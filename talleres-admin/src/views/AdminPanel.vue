  ...
  <div class="admin-wrap">
    ...
    <aside class="sidebar" :class="{ collapsed: sidebarCollapsed }">
      ...
      <div class="sidebar-footer">
        <button class="sidebar-link logout" @click="doLogout" :title="'Cerrar sesión'">
          <span class="logout-mark">↪</span><span v-if="!sidebarCollapsed">Cerrar sesión</span>
        </button>
      </div>
    </aside>
    ...
    <main class="admin-main">
      ...
      <div v-if="store.error" class="global-error">
        <strong>Error de conexión.</strong> {{ store.error }}
      ...
      <template v-else-if="activeTab === 'parts'">
        ...
        <div class="filter-bar">
          <input
            v-model="partSearch"
            type="search"
            placeholder="Buscar por pieza o referencia"
            class="filter-input"
          />
        ...
        <div class="stats-row">
          <div class="stat-chip available">
            <span class="status-dot" />{{ countByStock('available') }} disponibles
          </div>
          <div class="stat-chip low">
            <span class="status-dot" />{{ countByStock('low') }} pocas unidades
          </div>
          <div class="stat-chip out">
            <span class="status-dot" />{{ countByStock('out') }} sin stock
          </div>
        </div>
        ...
        <div class="results-bar">
          <span class="results-count">
            {{ filteredAdminParts.length }}
            {{ filteredAdminParts.length === 1 ? 'pieza' : 'piezas' }}
            <template v-if="partSearch || partCatFilter || stockFilter">
              · filtrado de {{ store.parts.length }}</template
            >
          </span>
          ...
          <table class="admin-table">
            <thead>
              <tr>
                ...
                <th class="sortable" @click="setSort('ref')">
                  Referencia <span class="sort-icon">{{ sortIcon('ref') }}</span>
                </th>
                <th class="sortable" @click="setSort('category')">
                  Categoría <span class="sort-icon">{{ sortIcon('category') }}</span>
                </th>
                <th>Compatibilidad</th>
                <th class="sortable" @click="setSort('stockStatus')">
                  Stock <span class="sort-icon">{{ sortIcon('stockStatus') }}</span>
                </th>
              ...
              <tr v-for="part in pagedParts" :key="part.id">
                <td>
                  <div class="thumb">
                    ...
                    <span v-if="part.media && part.media.length > 1" class="thumb-count">{{
                      part.media.length
                    }}</span>
                  </div>
                </td>
                <td>
                  ...
                  <div v-if="part.description" class="part-desc-preview">
                    {{ part.description.substring(0, 70)
                    }}{{ part.description.length > 70 ? '…' : '' }}
                  </div>
                </td>
                <td>
                  <code>{{ part.ref || '—' }}</code>
        ...
        <div class="cats-grid">
          <div v-for="cat in store.catDefs" :key="cat.id" class="cat-card">
            ...
            <div class="cat-actions">
              <button class="icon-btn edit" @click="openCatModal(cat)" title="Editar">
                Editar
              </button>
              <button class="icon-btn del" @click="confirmDelete('cat', cat)" title="Eliminar">
                Eliminar
              </button>
            </div>
          </div>
      ...
      <template v-else-if="activeTab === 'settings'">
        <div class="settings-wrap">
          <section class="settings-card">
            ...
            <h3>Contacto</h3>
            <p class="settings-description">Canales que aparecen en la web pública.</p>
            <div class="form-row">
            ...
            <div class="form-group">
              ...
              <input
                v-model="sf.address"
                type="text"
                placeholder="C/ Pola Nº 2, Selaya, Cantabria"
              />
            </div>
          </section>
          ...
          <section class="settings-card">
            ...
            <h3>Horario y respuesta</h3>
            <p class="settings-description">Disponibilidad comercial mostrada al visitante.</p>
            <div class="form-row">
              <div class="form-group">
            ...
            <div class="form-row">
              ...
              <div class="form-group">
                ...
                <input
                  v-model="sf.response_time_text"
                  type="text"
                  placeholder="Te respondemos en horario comercial"
                />
              </div>
            </div>
          </section>
          ...
          <section class="settings-card">
            ...
            <p class="settings-description">Contenido principal de presentación del negocio.</p>
            <div class="form-row">
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <input
                  v-model="sf.hero_tagline"
                  type="text"
                  placeholder="Agrícola · Ganadero · Forestal · Jardín · Ordeño"
                />
              </div>
            </div>
            <div class="form-group">
              ...
              <textarea
                v-model="sf.hero_message"
                rows="2"
                placeholder="Venta, reparación y recambios para maquinaria agrícola, ganadera, forestal y de jardín"
              />
            </div>
            <div class="form-group">
          ...
          <section class="settings-card">
            ...
            <h3>Indicadores de portada</h3>
            <p class="settings-description">Cifras destacadas para reforzar la confianza.</p>
        ...
        <div v-if="partModal.open" class="overlay" @click.self="partModal.open = false">
          <div class="modal-box">
            ...
            <div class="modal-body">
              ...
              <div class="form-group">
                ...
                <textarea
                  ...
                  placeholder="Descripción, características, notas…"
                  rows="3"
                />
              </div>
              ...
              <div class="form-group">
                ...
                <div v-if="mediaItems.length" class="media-grid">
                  ...
                  >
                    ...
                    <div v-if="item.type === 'image'" class="media-thumb">
                      ...
                      <div class="media-overlay">
                        <button
                          ...
                          :class="{ active: item.is_primary }"
                          title="Imagen principal"
                        >
                          ⭐
                        </button>
                        <button
                          class="media-action remove"
                    ...
                    <div v-else class="media-thumb media-video">
                      <div class="video-thumb-inner">
                        <img
                          v-if="getVideoThumb(item.url)"
                          :src="getVideoThumb(item.url)"
                          alt="Vídeo"
                        />
                  ...
                  <div
                    class="media-add-btn"
                    @click="$refs.multiImgInput.click()"
                    title="Añadir imagen"
                  >
                ...
                <div class="video-add-row">
                  ...
                  <button class="btn btn-outline" @click="$refs.videoFileInput.click()">
                    Subir vídeo
                  </button>
                </div>
              </div>
            ...
            <div class="modal-foot">
              <button class="btn btn-outline" @click="partModal.open = false" :disabled="saving">
                Cancelar
              </button>
              <button class="btn btn-secondary" @click="savePart" :disabled="saving">
                {{ saving ? 'Guardando…' : partModal.isNew ? 'Añadir pieza' : 'Guardar cambios' }}
              </button>
            </div>
          </div>
        </div>
          ...
          <div class="modal-box modal-sm">
            ...
            <div class="modal-body">
              ...
              <div class="cat-preview">
                <span
                  class="cat-badge"
                  :style="{
                    background: cf.color + '22',
                    color: cf.color,
                    borderColor: cf.color + '55',
                  }"
                >
            ...
            <div class="modal-foot">
              ...
              <button class="btn btn-secondary" @click="saveCat" :disabled="saving">
                {{ saving ? 'Guardando…' : catModal.isNew ? 'Crear categoría' : 'Guardar cambios' }}
              </button>
            </div>
          </div>
        </div>
      ...
      <Transition name="modal-fade">
        <div v-if="delConfirm.open" class="overlay" @click.self="delConfirm.open = false">
          <div class="modal-box modal-sm">
            ...
            <div class="modal-body">
              <p>
                ¿Eliminar <strong>"{{ delConfirm.name }}"</strong>?
              </p>
...
watch([partSearch, partCatFilter, stockFilter], () => {
  currentPage.value = 1
})

const totalPages = computed(() =>
  Math.max(1, Math.ceil(filteredAdminParts.value.length / PAGE_SIZE)),
)
...
function addLocalImage(file) {
  if (file.size > 5 * 1024 * 1024) {
    partModal.error = 'Una imagen supera 5MB.'
    return
  }
...
function onDropMulti(e) {
  Array.from(e.dataTransfer.files).forEach((f) => {
    if (f.type.startsWith('image/')) addLocalImage(f)
  })
}
...
function onVideoFileChange(e) {
  ...
  if (file.size > 100 * 1024 * 1024) {
    partModal.error = 'El vídeo no puede superar 100MB.'
    return
  }
  mediaItems.value.push({
    _key: Date.now(),
...
function setPrimary(idx) {
  mediaItems.value.forEach((m, i) => {
    if (m.type === 'image') m.is_primary = i === idx
  })
}
...
.nav-btn.active {
  background: rgba(35, 138, 117, 0.22);
  color: #e9fff9;
  box-shadow: inset 3px 0 0 #38a990;
  font-weight: 650;
}
...
.settings-card h3 {
  font-size: 1.02rem;
  color: var(--green-dark);
  margin: 0.15rem 0 0.25rem 2.3rem;
}
